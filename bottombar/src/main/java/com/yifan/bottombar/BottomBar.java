...
public class BottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    private void populateAttributes(Context context, AttributeSet attrs) {
        mPrimaryColor = UiUtil.getColor(getContext(), R.attr.colorPrimary);
        mScreenWidth = UiUtil.getScreenWidth(getContext());
        mTenDp = UiUtil.dpToPixel(getContext(), 10);
        mMaxFixedItemWidth = UiUtil.dpToPixel(getContext(), 168);
        ...
        try {
            ...
            int defaultActiveColor = isShiftingMode() ? Color.WHITE : mPrimaryColor;
    ...
    private boolean drawUnderNav() {
        return !isTabletMode
                ...
                && UiUtil.shouldDrawBehindNavbar(getContext());
    }
    ...
    private void initializeViews() {
        ...
        ViewCompat.setElevation(this, UiUtil.dpToPixel(getContext(), 8));
    ...
    private void determineInitialBackgroundColor() {
        if (isShiftingMode()) {
            defaultBackgroundColor = mPrimaryColor;
        }
    ...
    private void resizeTabsToCorrectSizes(List<BottomBarTab> bottomBarItems, BottomBarTab[] viewsToAdd) {
        int proposedItemWidth = Math.min(
                UiUtil.dpToPixel(getContext(), mScreenWidth / bottomBarItems.size()),
                mMaxFixedItemWidth
        );
    ...
    private void handleClick(View v) {
        ...
        boolean isHandleClick = true;
        if (onTabSelectListener != null) {
            isHandleClick = onTabSelectListener.preTabSelected(newTab.getId());
        }
        if (!isHandleClick) {
            return;
        }
